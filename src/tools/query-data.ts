...
export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("HPA_DATA_DO", "hpa");

    server.registerTool(
        "hpa_query_data",
        {
            title: "Query Staged HPA Data",
            description:
                "Query staged Human Protein Atlas data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
